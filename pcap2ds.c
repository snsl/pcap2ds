...
/** Send All g_log messages to our own handler **/
  ...
  if (cf_name) {
    ...
    TRY {
      ...
      err = load_cap_file(&cfile, global_capture_opts.save_file, out_file_type,
          global_capture_opts.has_autostop_packets ? global_capture_opts.autostop_packets : 0,
			  global_capture_opts.has_autostop_filesize ? global_capture_opts.autostop_filesize : 0, output_file_name);
#else
      err = load_cap_file(&cfile, NULL, out_file_type, 0, 0, output_file_name);
#endif
    }
    ...
    if (!write_preamble(output_file_name)) {
	      ...
	      gchar *output_file_name)
{
  ...
  if (save_file != NULL) {
    ...
    if(shb_hdr->shb_user_appl == NULL) {
        g_snprintf(appname, sizeof(appname), "TShark " VERSION "BETA");
...
static gboolean
write_preamble(gchar *output_file_name)
{
	void write_ds_preamble(gchar *);
	write_ds_preamble(output_file_name);
	return TRUE;
}

static gboolean
print_packet(epan_dissect_t *edt)
{
  void proto_tree_write_ds(epan_dissect_t *edt);
  proto_tree_write_ds(edt);
  return TRUE;
}
